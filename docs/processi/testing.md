...
| Personalizzazione Home | `test/features/home_widget_visibility_test.dart`, `test/widget/add_widgets_empty_state_test.dart` | contratto zero/uno/molti, copy/semantica/CTA e budget del nuovo asset Chigio. |
...
| Timesheet / cache offline | `test/features/timesheet/cache_row_test.dart` | mapping riga Drift → `DailyTimesheet`, degradazione tollerante di timestamp e `segments` corrotti e caratterizzazione del buco noto sui campi assenza (vedi `docs/funzionalita/timesheet.md`). |
